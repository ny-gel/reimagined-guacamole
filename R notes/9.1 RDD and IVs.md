...
air_scatter <- ggplot(
  data = air_data,
  aes(x=watts, y=aqi, color = group, shape = group)) +  #x = forcing_var, y=outcome_var, color = treatment_var
  geom_point() +
...
air_scatter2 <- air_scatter +
 geom_smooth(
  aes(group=group), #plot separate lines for each `group = treatment_var`
...
Using the correct cutpoint and optimal bandwidth calculated, we then fit a local linear regression model with `RDestimate()` function:
```r
...
To print the results of the local linear regression model, use `summary(local_model)` function:
```r
summary(air_rdd)
...
To know the number of observations at optimal bandwidth, half bandwidth and 2x bandwidth + standard errors, we can use:
```r
rdd_mod$obs
rdd_mod$se
```
