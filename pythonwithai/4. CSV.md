...
with open('passwords.csv','r') as password_file:
  ...
  for lines in password_csv:
    compromised_users.append(lines['Username'])

with open('compromised_users.txt','w') as compromised_user_file:
  for user in compromised_users:
    compromised_user_file.write(user + '\n')
    # Files opened in write cannot be read from, so write to file instead of iterating

with open('boss_message.json', 'w') as boss_message:
  boss_message_dict = {"recipient": "The Boss", "message":"Mission Success"}
  # In dicts, use colons (:) to separate keys and values, not equal signs
